...
class GAE(Critic):
    ...
    def gradient_update(
        ...
        with tf.GradientTape() as vf_tape:
            values = self.vf(observations)[:, :, 0] * tf.cast(
                tf.range(tf.shape(observations)[1])[tf.newaxis, :] < lengths[:, tf.newaxis],
                observations.dtype)
            vf_loss = tf.reduce_mean(tf.losses.mean_squared_error(
                returns, values[:, :(-1)]))
            if self.monitor is not None:
                self.monitor.record("returns_mean", tf.reduce_mean(returns))
                self.monitor.record("values_mean", tf.reduce_mean(values))
    ...
    def gradient_update_return_weights(
        ...
        values = self.gradient_update(
            ...
            rewards,
            lengths)
        delta_v = (rewards + values[:, 0:, 0] * self.gamma - values[:, :(-1), 0])
        weights = tf.tile([[self.gamma * self.lamb]], [1, tf.shape(delta_v)[1]])
        weights = tf.math.cumprod(weights, axis=1, exclusive=True)
        advantages = tf.math.cumsum(delta_v * weights) / weights
        if self.monitor is not None:
            self.monitor.record("advantages_mean", tf.reduce_mean(advantages))
        return advantages
